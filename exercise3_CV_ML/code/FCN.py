...
class FCN_SS(object):
    ...
    def build_train_graph(self):
        ...
        class_weight = tf.reshape(tf.convert_to_tensor(
            class_weight, dtype=tf.float32), [12])
        ...
        with tf.name_scope("compute_loss"):
            ...
            if opt.version_net == 'FCN_Seg':
                TotalLoss = loss(segMap, Reshaped_labels,
                                 self.N_classes, class_weight)
    ...
    def unfould(self, image):
        ...
        for row in range(0, self.height):
            for column in range(0, self.width):
                ...
                label_training[0, row, column, int(
                    image[0, row, column, 0])] = 1
                # else:
    ...
    def train(self, opt):
        ...
        sv = tf.train.Supervisor(logdir=opt.logs_path,
        ...
        with sv.managed_session() as sess:
            ...
            for step in range(1, opt.max_steps):
                ...
                if step % opt.summary_freq == 0:
                    ...
                    print("Epoch: [%2d] [%5d/%5d] time: %4.4f/it loss: %3f" % (train_epoch,
                                                                               train_step, opt.steps_per_epoch, time.time() - start_time, results["loss"]))
                ...
                # Saving model
                # and (Accuracy_validation < Global_IoU):
                if ((step % (NumberIterations_train_to_test*2)) == 0):
    ...
    def save(self, sess, checkpoint_dir, step):
        ...
        if step == 'latest':
            self.saver2.save(sess, os.path.join(
    ...
    def build_test_graph(self):
        input_img = tf.placeholder(tf.float32, [
                                   self.batch_size, self.img_height, self.img_width, 3], name='input_image')
